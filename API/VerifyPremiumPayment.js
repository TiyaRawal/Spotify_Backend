...
let VerifyPremiumPayment = async (req, res) => {
  try {
    ...
    let premiumExpiryDate = new Date();
    premiumExpiryDate.setMonth(premiumExpiryDate.getMonth() + 1);

    await collection.updateOne({_id: new ObjectId(req.user.id)},{
        $set: {
          premium: true,
          premiumPlan: plan,
    ...
    return res.status(200).send({message: "Premium Activated", premium: true, premiumPlan: plan, premiumDate: premiumDate, premiumExpiryDate: premiumExpiryDate,});
  } catch (e) {
